<?php  
	...
	//init default value
	if (empty($_POST)){
		$_POST['categori'] = 'NO_REKENING_PENGIRIM';
			...
			<div class="box">
							...
							<select name='noRek' >
								<?php
									...
									for ($i=0; $i<count($data); $i++){
										if (isset($_POST['noRek']) && $data[$i][0] == $_POST['noRek'])
					...
					<div class='field'>
						...
						<div class='right'>
							<select name='categori'>
								<?php
									//membuat daftar keyword, debet == terima. kredit == kirim
									$values = array("NO_REKENING_TUJUAN", "NO_REKENING_PENGIRIM");
									$text = array("Debet", "Kredit");
				...
				<?php 
					...
					echo "<table class='tabel'>";
						echo "<tr>";
							if ($_POST['categori'] == 'NO_REKENING_PENGIRIM'){
								echo "<td>No</td><td>No Rekening Tujuan</td><td>Nominal</td><td>waktu</td>";
							}else{
								echo "<td>No</td><td>No Rekening Pengirim</td><td>Nominal</td><td>waktu</td>";
							}
					...
					if (isset($_POST['cek'])){
						...
						WHERE {$_POST['categori']}=:key");
						$statement->bindValue(':key', $_POST['noRek']);
					}else{
						$statement = $dbc->prepare("SELECT transaksi.* FROM `transaksi` 
					...
					for ($row=0; $row<count($tmp);$row++){
						$cekKosong = False;
						if ($_POST['categori'] == 'NO_REKENING_PENGIRIM')  //kredit
